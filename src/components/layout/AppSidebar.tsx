...
import {
  Home, Users, Calendar, DollarSign, Sparkles, Package, Scissors,
...
const navItems = [
  ...
  { label: "ConsultProLash", icon: Sparkles, path: "/home_profissional", tab: "ConsultProLash" },
    ...
    <div className="flex flex-col h-full">
      ...
      <div className={cn("mx-3 mb-4 p-3 rounded-xl bg-secondary/50", collapsed && "mx-1 p-2")}>
        <div className={cn("flex items-center gap-3", collapsed && "justify-center")}>
          <div className="w-9 h-9 rounded-full gradient-pink flex items-center justify-center flex-shrink-0 text-sm font-bold text-primary-foreground">
            JS
          </div>
          {!collapsed && (
            <div className="min-w-0">
              <p className="text-sm font-semibold text-foreground truncate">Julia Soares</p>
              <p className="text-xs text-muted-foreground truncate">Lash Designer</p>
            </div>
          )}
        </div>
      </div>
        ...
        {navItems.map((item) => (
          <NavLink
            ...
            className={cn(
              ...
              isActive(item)
                ? "gradient-pink text-primary-foreground shadow-lg glow-pink"
